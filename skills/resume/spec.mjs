...
export const TABLES = { recovery: { reads: ["state.canonicalNext"], rows: [
...
] }, canonicalNext: { reads: ["intent.scope", "state.canonicalNext", "state.routeStage"], rows: [
  ...
  { state: "ASK:setup", reads: ["state.canonicalNext", "state.routeStage"], acceptsUnknown: ["state.routeStage"], when: s => s.state.canonicalNext === "setup.no-product" || s.state.canonicalNext === "setup.layer0-uncommitted" || (s.state.canonicalNext === "setup.layer0-incomplete" && s.state.routeStage !== "arch") || s.state.canonicalNext === "setup.brownfield-field" || s.state.canonicalNext === "git.open-operation" },
  { state: "ROUTE:arch", reads: ["state.canonicalNext", "state.routeStage"], acceptsUnknown: ["state.routeStage"], when: s => (s.state.canonicalNext === "setup.layer0-incomplete" && s.state.routeStage === "arch") || s.state.canonicalNext === "marker.glossary-term" || s.state.canonicalNext === "marker.design-note" || s.state.canonicalNext === "marker.design-open-item" || s.state.canonicalNext === "marker.knowledge-landing" || s.state.canonicalNext === "setup.integration-config" || s.state.canonicalNext === "baseline.legacy-v010" || s.state.canonicalNext === "baseline.design-refresh" },
...
export const DECLARATIONS = {
  ...
  single_route: { value: "Resume consumes route.id once after principles classification; a kernel-proven completed Product boundary and glossary-term, design-note, design-open-item, knowledge-landing, legacy-v010, and design-refresh route to Arch for every managed project, while setup.unmanaged remains DONE and unproven setup remains an ASK with no writer route.", consumer: "recovery-action|scope-entry" },
