...
export const TABLES = { recovery: { reads: ["state.canonicalNext"], rows: [
  ...
  { state: "ASK:setup", reads: ["state.canonicalNext"], acceptsUnknown: [], when: s => s.state.canonicalNext === "setup.no-product" || s.state.canonicalNext === "setup.layer0-uncommitted" || s.state.canonicalNext === "setup.layer0-incomplete" || s.state.canonicalNext === "setup.brownfield-field" || s.state.canonicalNext === "git.open-operation" },
...
export const DECLARATIONS = {
  ...
  single_route: { value: "Resume consumes route.id once after principles classification; glossary-term, design-note, design-open-item, knowledge-landing, legacy-v010, and design-refresh route to Arch for every managed project, while setup.unmanaged remains DONE and setup.layer0-uncommitted remains an ASK with no writer route.", consumer: "recovery-action|scope-entry" },
