...
export const GUARDS = [{ id: "unrecognized-state", reads: ["intent.scope", "state.canonicalNext"], when: s => s.intent.scope === "handoff-continuation" && s.state.canonicalNext === "unrecognized", then: "BLOCK", body: "guard: unrecognized-state" }];
export const STAGES = [
  { id: "recovery-action", reads: ["state.canonicalNext", "state.brownfield"], acceptsUnknown: [], done: s => !["marker.design-note", "marker.design-open-item", "baseline.boundary", "layer.folder-boundary", "ready.digest-behind"].includes(s.state.canonicalNext) && ["yes", "no"].includes(s.state.brownfield), table: "recovery", reentry: "rejudge", branches: { "route-arch": [["READ", { artifact: "cards" }], ["READ", { path: "references/recovery-policy.md" }], "ROUTE:arch"], "route-adopt": [["READ", { artifact: "cards" }], ["READ", { path: "references/recovery-policy.md" }], "ROUTE:adopt"], "finish-baseline-boundary": [["READ", { artifact: "cards" }], ["READ", { path: "references/recovery-policy.md" }], ["COMMIT", { boundary: "baseline" }], "WAIT"], "commit-folder-boundary": [["READ", { artifact: "cards" }], ["READ", { path: "references/recovery-policy.md" }], ["COMMIT", { boundary: "folder" }], "WAIT"], "commit-digest": [["READ", { artifact: "digest" }], ["READ", { path: "references/recovery-policy.md" }], ["COMMIT", { boundary: "digest" }], "WAIT"] }, body: "stage: recovery-action" },
  { id: "scope-entry", reads: [], acceptsUnknown: [], done: () => false, needs: ["intent.scope"], table: "canonicalNext", branches: { "DONE:orientation": [["REPORT", { template: "orientation" }], "DONE"], "ASK:integrity": ["ASK"], "ASK:setup": ["ASK"], "ASK:owner-mapping": ["ASK"], "ROUTE:product": ["ROUTE:product"], "ROUTE:arch": ["ROUTE:arch"], "ROUTE:adopt": ["ROUTE:adopt"], "ROUTE:split": ["ROUTE:split"], "ROUTE:work": ["ROUTE:work"], "ROUTE:verify": ["ROUTE:verify"], "WAIT:blocked": [["REPORT", { template: "result" }], "WAIT"], "DONE:advisory": [["REPORT", { template: "result" }], "DONE"], "DONE:complete": [["REPORT", { template: "result" }], "DONE"], "BLOCK:unrecognized": ["BLOCK"] }, reentry: "rejudge", body: "stage: scope-entry" }
];
export const TABLES = { recovery: { reads: ["state.canonicalNext", "state.brownfield"], rows: [
  { state: "route-adopt", reads: ["state.canonicalNext", "state.brownfield"], acceptsUnknown: [], when: s => (s.state.canonicalNext === "marker.design-note" || s.state.canonicalNext === "marker.design-open-item") && s.state.brownfield === "yes" },
  { state: "route-arch", reads: ["state.canonicalNext", "state.brownfield"], acceptsUnknown: [], when: s => (s.state.canonicalNext === "marker.design-note" || s.state.canonicalNext === "marker.design-open-item") && s.state.brownfield === "no" },
...
] }, canonicalNext: { reads: ["intent.scope", "state.canonicalNext", "state.brownfield"], rows: [
  { state: "DONE:orientation", reads: ["intent.scope"], acceptsUnknown: [], when: s => s.intent.scope === "domain-orientation" },
  { state: "ROUTE:verify", reads: ["state.canonicalNext"], acceptsUnknown: [], when: s => s.state.canonicalNext === "transition.prepared-route" || s.state.canonicalNext === "transition.interrupted" || s.state.canonicalNext === "transition.source-id-migration" || s.state.canonicalNext === "transition.product-running" || s.state.canonicalNext === "transition.product-result" || s.state.canonicalNext === "transition.event-routing" || s.state.canonicalNext === "transition.event-decision" || s.state.canonicalNext === "transition.failure-routing" || s.state.canonicalNext === "marker.capability-closure" || s.state.canonicalNext === "event.product-requested" || s.state.canonicalNext === "event.pending" || s.state.canonicalNext === "event.new" || s.state.canonicalNext === "layer.children-done" || s.state.canonicalNext === "product.shape-or-revision" || s.state.canonicalNext === "product.fail" || s.state.canonicalNext === "product.unverified" },
  ...
  { state: "ROUTE:split", reads: ["state.canonicalNext"], acceptsUnknown: [], when: s => s.state.canonicalNext === "transition.layer-opening" || s.state.canonicalNext === "marker.re-split" || s.state.canonicalNext === "claim.depends-anomaly" || s.state.canonicalNext === "claim.needs-reapproval" || s.state.canonicalNext === "claim.blocked-by-prerequisite" || s.state.canonicalNext === "request.existing" || s.state.canonicalNext === "layer.empty-folder" || s.state.canonicalNext === "layer.correspondence-gap" || s.state.canonicalNext === "layer.no-foundation" || s.state.canonicalNext === "layer.no-tree" || s.state.canonicalNext === "ready.needs-normalization" || s.state.canonicalNext === "ready.approval-invalid" || s.state.canonicalNext === "ready.approval-pending" || s.state.canonicalNext === "ready.waiting-capability" },
  ...
  { state: "ASK:owner-mapping", reads: ["state.canonicalNext"], acceptsUnknown: [], when: s => s.state.canonicalNext === "marker.glossary-term" },
  { state: "ROUTE:adopt", reads: ["state.canonicalNext", "state.brownfield"], acceptsUnknown: [], when: s => (s.state.canonicalNext === "baseline.legacy-v010" || s.state.canonicalNext === "baseline.design-refresh" || s.state.canonicalNext === "marker.knowledge-landing") && s.state.brownfield === "yes" },
  { state: "ROUTE:arch", reads: ["state.canonicalNext", "state.brownfield"], acceptsUnknown: [], when: s => s.state.canonicalNext === "setup.integration-config" || ((s.state.canonicalNext === "baseline.legacy-v010" || s.state.canonicalNext === "baseline.design-refresh" || s.state.canonicalNext === "marker.knowledge-landing") && s.state.brownfield === "no") },
...
export const DECLARATIONS = {
  ...
  single_route: { value: "Resume consumes route.id once after principles classification; it never recreates a state predicate from cards, journals, dependencies, CLI text, or a handoff.", consumer: "scope-entry" },
  ...
  landing_boundary: { value: "Any remaining knowledge-landing marker keeps its arch or adopt owner active, including partial multi-owner landing.", consumer: "scope-entry" }
};
