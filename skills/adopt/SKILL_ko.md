...
- **캡슐 가공.** 사용자가 지목한 기존 도메인 문서가 능력 문서 예산을 넘치면 기준선 판정
  ...
  좌표에서 실제로 찾는 출처 표본 검사를 포함하며, 실패한 캡슐은 착지하지 않는다 ⑤아래 설계
...
- 바뀔 설계 구역 전부를 디스크에 쓰기 전에 한 묶음으로 제시하고 사용자 확인을 받는다. 확인
  전에는 능력 문서 경로를 바꾸지 않는다. 확인 뒤 바뀐 능력 문서와 그 지식 캡슐만
  `adopt — capabilities` 커밋 하나에 담는다. 그 커밋은 이 실행의 마지막 커밋이며 Layer 0
  파일을 함께 싣지 않는다. 바뀐 파일이 없으면 확인을 묻거나 커밋하지 않는다.
