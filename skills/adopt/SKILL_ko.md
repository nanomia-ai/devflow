...
- `devflow/project/`에 product.md·code-style.md·glossary.md와 `브라운필드` 필드가 있는
  ...
  `Design head`가 있으면 일이 없다 — 이어하기는 resume이다. Layer 0는 완성됐지만 기대 파일이
  없거나 설계 구역·Design head만 낡았으면 1~5단계를 다시 실행하지 않고 아래 `능력 문서` 절만
...
- 그 밖의 파일은 정본의 초기 생성 정의상 없으면 초기 검증 구역까지 만든다. 고정 경계가 하나인 기존 파일은 검증 구역 바이트를
  ...
  서술의 폐기와 초기화를 선택하지 않았으면 쓰지 않고 `기준선 무연산: <경로와 이유>`를 보고한다.
