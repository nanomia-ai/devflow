...
## guard: approval-precedes-refutation

An approval that arrives while refutation is not currently clear belongs to a proposal this run has not presented. Drop that value, finish the refutation, and let the write-free proposal ask again.
...
Why: one confirmation binds the complete write set, so the proposal it answers must already be complete and currently refuted.
...
Block only on a departure from the opened contracts, or a supported contradiction or omission that would cause a wrong action, lose a required decision or rejected direction, assign the wrong owner, omit or split a knowledge unit's current landing, or omit verification means. On a correction, recheck only the returned coordinates, changed targets, and directly affected inventory landing, reference, owner, and source relationships; open no new full pass. Never author, correct, or approve the draft. Return `clear`, `revise`, or `blocked` with the preserved initial coverage, the current causal scope, each finding's coordinate and blocking status, and the remaining uncertainty.
