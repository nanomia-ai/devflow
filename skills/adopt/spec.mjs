...
export const OBSERVATIONS = {
  ...
  "refutation.state": { judged: true, domain: ["pending", "clear", "blocked"] }
};
...
export const STAGES = [
  { id: "semantic-refutation", reads: ["refutation.state"], acceptsUnknown: [], done: s => s.refutation.state === "clear", needs: ["refutation.state"], reentry: "rejudge", branches: {
    pending: [["READ", { path: "references/workflow.md" }], ["RUN", { action: "inventory-all-maintained-code-docs-specs-and-records-with-exact-coordinates-and-one-disposition-each" }], ["RUN", { action: "trace-one-executable-flow-per-code-backed-capability-candidate-and-record-documentary-basis-for-document-derived-candidates" }], ["RUN", { action: "reverse-derive-complete-layer-zero-applicable-design-capability-zones-glossary-and-all-required-owner-adjacent-knowledge" }], ["RUN", { action: "perform-one-bounded-clean-context-semantic-refutation-of-the-complete-draft-before-binding" }], "NEXT"],
    blocked: ["BLOCK"]
  ...
  { id: "adoption", reads: ["state.route"], acceptsUnknown: [], done: s => s.state.route !== "setup.unmanaged", table: "approval", reentry: "rejudge", branches: {
    ...
    approve: [["WRITE", { artifact: "architecture", template: "architecture" }], ["WRITE", { artifact: "design", template: "design", selection: "when-existing-product-evidence-has-a-design-surface", zeroAllowed: true }], ["WRITE", { artifact: "codeStyle", template: "codeStyle" }], ["WRITE", { artifact: "glossary", template: "glossary" }], ["RUN", { action: "append-principles-maintenance-routing-pending-idempotently-with-the-whole-follow-on-request-when-adoption-includes-follow-on-work" }], ["WRITE", { artifact: "product", template: "product", ordering: "last-owner-document-write-before-first-commit" }], ["WRITE", { artifact: "knowledgeNodes", template: "knowledgeNode", selection: "every-approved-layer-zero-owner-body-needing-depth-beyond-its-owner-document", zeroAllowed: true }], ["RUN", { action: "validate-every-written-layer-zero-knowledge-node-with-project-knowledge" }], ["COMMIT", { boundary: "confirmed-brownfield-layer-zero", staging: "only-layer-zero-documents-their-knowledge-nodes-and-follow-on-paths" }], ["RUN", { action: "calculate-canonical-design-head-from-the-landed-layer-zero-commit" }], ["WRITE", { artifact: "capabilityDesigns", template: "capabilityDesign", fields: { designHead: "canonical-design-head-output" } }], ["WRITE", { artifact: "knowledgeNodes", template: "knowledgeNode", selection: "every-approved-capability-body-needing-depth-beyond-its-capability-document", zeroAllowed: true }], ["RUN", { action: "validate-every-written-capability-knowledge-node-with-project-knowledge" }], ["COMMIT", { boundary: "confirmed-brownfield-capabilities-and-knowledge", staging: "only-capability-design-documents-and-their-knowledge-nodes" }], ["REPORT", { template: "result" }], "DONE"]
  }, body: "stage: adoption" }
];
...
export const DECLARATIONS = {
  ...
  source_coverage: { value: "Every maintained source receives one explicit disposition: landed in a named Product, Architecture, Design, glossary, capability, or K owner; retained as a supporting source with exact coordinates; superseded or contradicted by named evidence; or excluded as non-domain with a reason. The approved coordinates, authority status, disposition, and landing owner persist in Architecture Existing records; no maintained source is silently dropped and no second ledger is created.", consumer: "template:adoptionProposal|template:architecture|stage:adoption" },
  semantic_refutation: { value: "Before binding, run one bounded clean-context semantic refutation over the complete draft, selected authority coordinates, and the same shared baseline and knowledge sections opened by workflow step 5. A departure from those sections, or a supported contradiction or omission that would cause a wrong action, lose a required decision or rejected direction, assign the wrong owner document, or omit the means of verification, blocks. Revise at most once, recheck only returned coordinates, record scope, findings, and result in Evidence verification, and do not bind while a blocking finding or unavailable refutation remains.", consumer: "template:adoptionProposal|stage:adoption" },
  brownfield_effects: { value: "Evidence and the complete proposal precede owner questions and one binding approval; refusal writes nothing. Approval first commits complete confirmed Layer 0 with its owner documents' validated K nodes and any optional follow-on record as adopt — layer 0 with product.md written last among owner documents, then writes capability design documents with Design head equal to that landed commit, writes and validates their K nodes beside them, and commits both as adopt — capabilities. Interruption remains unmanaged only while no current .devflow root or indexed path exists; once current evidence exists canonical state owns recovery. A post-product pre-commit interruption is reported for owner-directed Git recovery; after the first commit and before second-boundary writes, baseline-missing state is recovered by Resume to Arch, never by Adopt.", consumer: "stage:adoption" },
  ...
  knowledge_shape: { value: "K is created only when selected domain sources overflow the always-read owner document — Product, Architecture, Design, or a capability; it has subtree-unique immutable number, exact source basis, zero children allowed, and no Parent, index, residual, or batch metadata.", consumer: "template:knowledgeNode|stage:adoption" }
};
