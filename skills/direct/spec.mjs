...
export const STAGES = [
  ...
  { id: "materialize", reads: ["planning.receipt", "origin.drafts"], acceptsUnknown: [], done: s => s.planning.receipt !== "NONE" || s.origin.drafts !== "NONE", needs: ["bundle.contract", "bundle.units"], table: "materializeSelector", reentry: "rejudge", branches: {
    "continue-active-bundle": [["RUN", { action: "project existing K headers for bounded current Layer 0, target capability, and explicit crosscut owners; add only destination-matched exact K paths to each card Read first" }], ["WRITE", { artifact: "cardBundle", target: "<exact-active-scopes>/<one-or-more-sibling-card-addresses>", units: "bundle.units", templates: "00-project=researchCard; all-other-scopes=taskCard", forbiddenTemplates: "taskCard@00-project" }], "NEXT"],
    "begin-request-bundle": [["WRITE", { artifact: "layerOpeningBundle", target: ".devflow/journal.md", source: "request.current.source", scopes: "bundle.units" }], ["COMMIT", { scope: "layer-opening-bundle", message: "direct — begin <parent>" }], ["RUN", { action: "project existing K headers for bounded current Layer 0, target capability, and explicit crosscut owners; add only destination-matched exact K paths to each card Read first" }], ["WRITE", { artifact: "cardBundle", target: "<exact-active-scopes>/<one-or-more-sibling-card-addresses>", units: "bundle.units", templates: "00-project=researchCard; all-other-scopes=taskCard", forbiddenTemplates: "taskCard@00-project" }], "NEXT"],
