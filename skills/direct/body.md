...
Before writing cards, project only existing K headers under the current Layer 0 owner needed by the unit, its target capability, and any explicit crosscut owner. Match first-line use-when against the card Destination and target; `Affected knowledge owners` narrows candidates but is not the sole read gate. Put only matching exact K paths in `Read first`, reproject only a needed nested branch, and never open bodies or enumerate other capabilities. If the tool is unavailable or the projection exceeds its budget, report that and keep the card narrow without guessing a path. Reuse a principles-projected active marker bundle without remapping it, never create knowledge-landing markers, and never write K.
