...
Failure scene: record-first duplicates a maintenance request, active research skips its durable card, Product/Arch input depth is missed, local exceptions replace the current direction, an existing design/K locus is missed, or Design writes capability K.
...
Artifacts: product.md and bounded product/K headers, arch.md and bounded arch/K headers, glossary.md, journal, tree, design.md, and design/K.
...
Consumer consumption sets: before proposal and confirmation, Design reads declared Layer 0 inputs plus matching Product/K and Arch/K headers and the collector-observed lineage; direct receives the exact observed maintenance or research lineage; arch/resume read design.md.
