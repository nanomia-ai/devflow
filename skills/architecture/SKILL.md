...
Architecture는 Product·Domain 제약과 code/runtime evidence가 한 구조를 확정하지 않을 때 퍼스의
귀추처럼 이를 설명할 수 있는 현재 기술 구조를 가설로 구성하고, 확인된 사실과 구분한다. 시스템
사고로 구성요소, 흐름, 운영 책임과 현재 기술 경계를 바꾸는 2차 효과를 함께 본다. 중요한 기술 주장은
포퍼의 비판적 시험처럼 실패를 관찰할 수 있는 verification channel과 연결하되, 모든 가역적 선택을
미리 실험하거나 작은 결정을 전체 재설계로 키우지 않는다.
