...
decision을 만들거나 교체할 때, 그 결정이 보호하는 규칙의 canonical home 또는 가장 가까운 parent에
조건부 route를 함께 둔다. 독자가 그 규칙을 바꾸거나 기각한 대안을 다시 들이려 할 때, 또는 재검토
조건에 닿는 변화가 생겼을 때 이 decision을 읽게 한다. 이유를 양쪽에 복제하지 않는다.
