...
1. 진행 로그가 비어 있지 않은 카드면, 바꾸기 전에 카드를 체크포인트로 커밋한다
   (`NN.N wip:` — 로그의 git 보증은 이 커밋이 만든다. 다중: 내 브랜치로 충분)
2. 카드 파일을 **같은 번호·같은 이름의 폴더**로 바꾼다
3. 원래 카드의 목적지·왜·금지·의존과 이월된 인용은 자식 카드들에 분배한다 — 폴더
   자체는 카드를 갖지 않는다 (폴더의 의미는 이름과 자식들이 말한다. 능력 폴더와 같은 원리)
4. 자식 번호 = 부모 번호 + 한 자리 (`02.3` → `02.3.1`). 번호 불변 규칙은 그대로
5. 카드에 점유 접미사(`.wip.`, 다중: `.wip-<내 id>.`)가 있었다면: 점유자가 이어서
   잡을 자식 하나가 승격 커밋 안에서 그 접미사를 단 채 태어난다 — 별도 점유 커밋
   없음(승격 커밋이 점유 가시화를 겸한다)
6. 진행 로그가 있었다면: 결론·제약은 영향받는 자식 카드들의 `읽을 것`으로, 진행분은
   5의 자식이 있으면 그 진행 로그로, 없으면 진행분이 속한 자식의 `읽을 것`에
   인용으로 옮긴다 — 파일이 사라지므로 포인터가 아니라 내용을 옮긴다
7. 승격은 미루지 않는 별도 커밋이다 (`NN.N promote`. 다중: 구속 결정 — 통합에 착지)
...
- 조사 카드는 좌표·정체성을 갖지 않는다 (규칙 정본 정체성 반복 주입의 공인된 예외 —
  답이 나오면 로그가 동결되는 카드라 주입의 효용이 없다). 의존·읽을 것·등급은
  필요한 만큼 그대로 쓴다.
...
다중: **번호 발급은 능력 폴더별로 한 번에 한 사람이다** (승격의 자식 번호도 발급이다 —
승격 커밋 착지로 선다). 시작을 journal 1줄로 선언한다
...
실행 제안에서 "검토 생략"을 명시해 뺄 수 있다. 조사 카드는 이 명시가 필요 없다 —
처음부터 검토 대상이 아니다 (work의 검토 규칙과 동일 — 산출물이 답이지 코드가
아니다. 본 코드에 diff가 생겼다면 검토를 받는다).
