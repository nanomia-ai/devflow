...
`capability_baseline`도 같은 묶음에서 한 질문으로 묻는다 — 대형·장수 서비스는 yes, 소형
MVP는 no를 추천한다.
...
capability_baseline: yes | no       # 부재 = no. yes면 모든 능력의 능력층 폐쇄가 그 능력의 능력 지식 기준선을 갱신한다
...
`capability_baseline`은 능력 지식 기준선을 켜는 값이다. `yes`면 모든 능력의 능력층 폐쇄가
`devflow/project/capabilities/`에 있는 그 능력의 기준선을 갱신하고, 부재는 `no`다. 값을 바꾸는
것은 발견→갱신 표의 arch 행으로 착지하는 구속 결정이며, 판정 전문은 기준선 판정 정본이
소유한다.
