...
- 능력 문서 예산을 넘치는 도메인 지식은 기준선 판정 정본의 지식 캡슐로 내린다. 원문 문서의
  가공은 adopt의 캡슐 가공 절차를 같은 계약으로 쓰고, 캡슐은 아래 설계 확인 묶음과 같은
  커밋에 동승한다.
...
- 바뀔 설계 구역 전부를 디스크에 쓰기 전에 한 묶음으로 제시하고 사용자 확인을 받는다. 확인
  ...
  `arch — capabilities` 커밋 하나에 담는다. 이 커밋은 이 실행의 마지막 커밋이며 Layer 0
  경로를 함께 싣지 않는다. 바뀐 파일이 없으면 확인을 묻거나 커밋하지 않는다.
