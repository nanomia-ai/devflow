...
Land product's attributed technical-choice lines beside their owning architecture conclusions. Project existing arch/K headers and report any changed current locus with the complete proposal before asking for explicit approval. Refusal and interruption before approval write nothing. On approval, write architecture, code style, only qualifying ADRs, and changed arch/K, validate that batch, then immediately land the complete Layer 0 boundary. A refresh uses its binding architecture boundary.
...
For a managed project, generate the foundation plus every non-retired product capability using the disk-first number and name rules. Rederive the entire expected design set from confirmed product, architecture, glossary, and only currently cited ADRs. Preserve an existing valid verified zone byte-for-byte; a new document receives the empty scaffold in the capability template. Project K headers only under expected changed owners, present the changed design-zone/K batch, and process a named source into K under the shared capsule contract; K rides the same confirmed capability-design commit.

A design note or design open item updates only its named capability design zone and any existing K that owns the changed unit, then deletes the exact routed line in that same validated binding commit. The note's anchor is the supplied snapshot basis; an open item's confirmed statement is its basis and its card is only the confirmation location.
