...
This mandatory grammar owns the canonical forms that P2's line DSL cannot parse when a top-level JSON string precedes another field. Render every line byte-for-byte; `project-state.mjs` is the deterministic parser and rejects malformed, reordered, duplicate, missing, bare, and CRLF forms.
