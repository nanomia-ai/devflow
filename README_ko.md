...
사람이 몰라서 틀릴 함정이 남는가. 기획·아키텍처·디자인·코드 스타일 문서가 있으면 읽고,
문구나 이름을 바꾸는 변경이면 용어집도 본 다음, 고친 뒤 확인
...
```
devflow/
  project/                     ← 자주 안 바뀌는 상위 문서
    product.md                    Layer 0 · 무엇을 왜 만드나 (능력 목록·성공 판정)
    arch.md                       Layer 0 · 어떻게 만드나 (스택·구조·잠정값·검증 창구)
    design.md  code-style.md      Layer 0 · (선택) 디자인 · 코드 취향 선언
    glossary.md  decisions/       Layer 0 · 용어집 · ADR
    capabilities/                 Layer 0 뒤 — 번호로 찾는 능력 문서, 설계 구역 + 검증 구역
...
이름이 오해를 부르니 분명히 해 둔다. **HANDOFF.md는 인수인계 문서가 아니다.** 트리가 다시
계산해내는 값 하나를 담은 캐시이고, 카드가 하나도 없으면 `none`이 정답이며 resume이 그것을
정상으로 규정한다. **digest.md도 요약이 아니다.** 남의 커밋을 어디까지 따라잡았는지 표시하는
커밋 해시 한 줄이다. 둘 중 어느 것도 사람이 읽을 서술을 담지 않는다.

### 기획 도중 컨텍스트가 차면

product·arch는 한 세션에서 끝나지 않아도 된다. 확정된 문서까지 커밋하고 세션을 끝내면 된다.
문서 하나가 확정될 때마다 자기 커밋으로 착지하므로, 그 경계에서 끊긴 것은 손실이 아니다.
다음 세션은 resume으로 들어온다 — 남은 것이 무엇이든 resume이 지목한다.

주의할 지점은 하나다. **arch는 arch.md와 code-style.md를 착지시킨 뒤에도 아직 끝난 것이 아니다.**
능력 문서를 만드는 마지막 단계가 남아 있고, 그것이 arch의 산출 중 가장 크다. arch는 그 단계를
시작하기 전에 남은 크기를 말하고, 컨텍스트가 부족하면 시작하지 않고 멈춘다. 그때는 점유한 카드가
없는 상태에서 새 세션이 resume으로 들어와 그 단계만 실행하면 된다.
...
고치면 둘 다 살아남고, 같은 자리를 고치면 조용히 덮는 대신 시끄럽게 실패한다. 알아 둘 것이
하나 있다. 커밋은 파일의 최종 내용을 담는다. 두 세션이 한 파일을 같이 고친 채 한쪽이 먼저
커밋하면 남의 몫까지 실린다. 그래서 경량 절차는 고치기 전에 대상 파일을 보고, 남의 미완성
수정이 있으면 물러난다. 사람이 지킬
...
되지만, 첫 디스패치 전에 `skills/principles/coordinator.md`를 읽어야 하고 카드 발급·점유·관문·
통합을 비롯한 의무 전부는 그 역할 계약 하나가 정한다.
...
| 같은 파일을 동시에 고치는 두 세션의 커밋 귀속 | 커밋은 파일의 최종 내용을 담지 수정의 주인을 모른다. 경량 절차의 사전 확인이 흔한 경우를 막지만, 확인과 커밋 사이의 짧은 틈은 잠금 없이는 닫히지 않는다 |
