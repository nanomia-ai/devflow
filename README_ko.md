...
devflow/
  ...
  users/<id>/                  ← 내 방 — owner.md(신원) · digest.md(마커) · HANDOFF.md(다음 한 걸음과 열린 결정, 매번 덮어씀)
```
...
- **핸드오프는 다음 걸음과 열린 결정만.** 위치는 트리가, 과정은 진행 로그가, 카드가 배운 것은
  그 카드의 승계 줄이 답한다. HANDOFF에는 다음 한 걸음과 사람이 결정할 것만 담는다.
...
**흐름 둘을 동시에 돌리는 깔끔한 방법은 워크트리다.** 한 저장소의 워크트리들은 이력 저장고
하나를 공유하므로 한 폴더의 점유가 원격도 fetch도 없이 다른 폴더에서 보이고, 폴더마다 미커밋
파일이 따로 있어 한쪽의 반쯤 짠 코드가 다른 쪽 테스트를 깨뜨리지 못한다. devflow는
`git worktree list`를 읽어 각 항목을 살아 있는 흐름으로 다룬다. 알아 둘 한계는 둘이다. Git은
한 워크트리가 다른 워크트리의 체크아웃 브랜치에 쓰는 것을 막으므로, 구속 결정은 통합 브랜치를
가진 워크트리에서 착지한다. 그리고 한 사람의 두 세션은 같은 id를 쓰므로 디스크가 둘을 구분하지
못한다 — 능력이 여럿 열려 있는데 어느 쪽인지 말하지 않으면 resume이 추측하지 않고 묻는다.
