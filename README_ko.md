...
터미널 두 개나 워크트리 두 개가 서로의 진행 중인 작업을 보게 만든다. 워크트리에는 단서가 둘
있다. 하나는 통합 브랜치에 있어야 한다 — Git이 다른 워크트리가 체크아웃한 브랜치를 갱신해 주지
않는다. 그리고 한 사람의 두 세션은 같은 id를 쓰므로 디스크가 둘을 구분하지 못한다 — 어느 쪽이든 다른
쪽의 경계를 끝낼 수 있고, 같은 카드를 집어드는 것을 막는 것은 resume 보고의 승인 절차다.
