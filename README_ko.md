...
devflow/
  ...
  journal.md                   ← 작업 간 결정 한 줄씩 + 규칙 정본이 정한 고정 형식 상태 줄 (정리되는 것은 결정뿐)
...
| `project/capabilities/NN-*.md` | verify, 능력이 통과로 닫힐 때 | 카드가 지목한 work, 도메인에 들어가는 사람 | 통과로 닫힐 때마다 통째로, 그리고 능력 개명 때 |
...
| `HANDOFF.md` | work, 작업 경계에서 | resume, work | 경계마다 통째로 덮어쓴다 |
| `users/<id>/` | 그 사람 | 본인, 남은 신원 확인용으로 owner.md만 읽는다 | 합류·점유·이탈 |

devflow가 남기는 상태 파일은 이 표가 전부다. 코드와 저장소의 다른 문서는 그대로 프로젝트의
것이다.

트리를 여는 두 관문, split과 resume은 시작하자마자 16항목 정합성 점검을 돌린다. 솔로
모드에서도 돈다. 중복된 번호, 닫힌 폴더 안에 살아 있는 카드, 어느 카드도 가리키지 못하는
...
| resume | 새 세션 | 상태 보고와 승인. 능력이 아닌 폴더는 가장 깊은 곳부터 스스로 닫아 경계 정리 커밋 하나로 착지시킨다 (다중: 소화 절차가 공유 문서를 정정한다) | HANDOFF와 트리가 충돌하면 트리가 이긴다 |
| principles | 나머지 여덟 스킬이 실행 전에 읽는다 | — | 규칙의 정본은 한 곳에만 산다 |

역할 넷이 동행한다. 서로의 영역을 침범하지 않는 것이 편향을 막는 장치다.

- **reviewer** — 작업 커밋 전에 카드(진행 로그 제외)·diff와 존재하는 code-style.md·glossary.md·journal.md만 읽고 판정한다. 실행하지 않는다. 검토를 면제할 수 있는 것은 사용자뿐이고, 코드 diff가 없는 조사 카드만 그 밖의 예외다.
...
- **확정 문장의 반증** — 측정이 확정된 정체성 문단이나 성공 판정을 무너뜨려도, 사용자 확인 없이는
  그 문장을 바꾸지 않는다.
- **능력이 둘로 보일 때** — 기준선이 같은 절이나 총 상한을 두 번 연속 넘기면 능력을 나눌지 묻는다.
  나눌지 말지는 사용자가 정한다.
...
**rename 커밋**으로 한다, 라우팅 전에 통합 브랜치를 fetch해 그 팁에서 공유 상태를
읽는다(거기 활성 마커가 있으면 통합 팁을 현재 브랜치에 먼저 포함한다). 앞의 둘은
경계에서만 돌고, 셋째는 점유 중에도 돈다. 나머지는 솔로와 같다.
...
전환이 덜 끝난 상태(다중 모드인데 무기명 `.wip.`이나 루트 HANDOFF가 남음)도 위의 정합성
점검이 잡아내는 항목이다. 조용히 잘못되는 일은 없다. 세션은 git 신원으로 자기
...
확인된 뒤에만 0.9.20 이전(그리고 0.9.0 이전 `nano-devflow`) 설치가 남긴 전역 훅 등록을
제거한다. 설치가 실패하면 기존 훅은
