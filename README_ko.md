...
| verify | 능력 완료 · MVP 도달 | verify.md · (실패 시) 수정 카드 · (사건 발동) 감리 소견 | 실행하지 않은 것은 통과가 아니라 미검증이다 |
...
역할 셋이 동행한다 — 서로의 영역을 침범하지 않는 것이 편향을 막는 장치다:
...
세 역할의 조건은 각각 스킬 옆의 계약 파일 하나다(`skills/work/reviewer.md` ·
`skills/verify/verifier.md` · `skills/verify/auditor.md`) — 어느 플랫폼에서든 깨끗한
서브에이전트/새 세션에 그 파일을 원문 그대로 브리핑해 수행하므로 **처리가 같다**.
