...
에이전트 2개가 동행한다 (Claude 전용 — Codex 설치에는 포함되지 않는다): **reviewer**는
커밋 전에 카드(진행 로그는 제외)·diff·code-style.md만 읽고 판정한다(실행하지 않음).
**verifier**는 구현 이력을 모른 채 실행만으로 판정한다(읽지 않음). 서로의 영역을
침범하지 않는 것이 편향을 막는 장치다.
