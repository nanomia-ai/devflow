...
- **retrospector(회고)** — devflow 산출물만 읽고 실행하지 않는다. MVP 경계에서 설계에 더 나은 선택지가 있었는지 소견으로만 평가한다 (아래 회고 문단).
...
일하므로, "더 나은 선택지가 있지 않았나"는 아무도 묻지 않는다. MVP에 도달하면 회고가
한 번, 코드를 읽지 않고 프로젝트가 남긴 흔적만으로 — 수정 카드가 몰린 폴더, `.stale.`
카드, ADR의 갱신 주석 — 그 물음을 던진다. 소견은 구체적 대안과 이 프로젝트의 긴장
증거를 갖출 때만 성립하고, 채택은 사용자의 것이다.
...
flowchart TB
    ...
    VF -.->|"제품층 첫 판정 뒤 1회 · 요청"| RT{{"retrospector · 회고 — 소견"}}
