...
**한 폴더에서 터미널 둘을 돌려도 안전하다.** 세션마다 자기 카드를 잡고 동시에 진행한다.
근거는 측정 셋이다. 커밋에 자기 경로를 지정하면 남이 미리 stage해 둔 파일이 섞이지 않는다.
네 세션이 동시에 기록장에 덧붙여도 한 줄도 잃지 않는다. 같은 소스 파일의 다른 부분을 각자
고치면 둘 다 살아남고, 같은 자리를 고치면 조용히 덮는 대신 시끄럽게 실패한다. 그래서 규칙은
하나뿐이다. 살아 있는 흐름이 또 있으면 파일을 통째로 다시 쓰지 말고 바꿀 부분만 고친다.
...
때문이다. 그리고 한 사람의 두 세션은 같은 id를 쓰므로 디스크가 둘을 구분하지 못한다 — 능력이
여럿 열려 있는데 어느 쪽인지 말하지 않으면 resume이 추측하지 않고 묻는다.
