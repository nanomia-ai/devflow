...
| DD-79 | README는 사람의 문서이고 AI의 읽기 집합 밖에 산다 — 읽지도 갱신하지도 판정 근거로 삼지도 않으며, 이 선은 README가 돌아온 뒤에도 그대로다 | 정체성 · 배포 · 플랫폼 | v0.18.5 | 유효 |
| DD-80 | 기계 판정의 실행자는 코드이고, 그것을 적은 문서는 마지막 런타임 독자가 옮긴 다음 릴리스에서 삭제된다 | 정체성 · 배포 · 플랫폼 | v0.18.7 | 유효 |
...
| DD-68 | 신호 카드는 완료된 수리의 후속 non-pass를 같은 root에 연결하고 직전 수리 증거를 상속하며 재관측 2 이상에서 사람에게 돌아간다 | 검증과 역할 | v0.15.0 | 유효 |
...
| DD-25 | 준비된 카드의 정의·카드별 실행 제안 승인·재개 라우팅은 디스크 상태로 판정한다 | 작업 트리와 카드 | v0.9.21 | 유효, DD-80이 부분 교정 (v0.18.7) |
...
| DD-69 | design은 여섯 Layer 0 결정을 확정하고 구축은 split 카드가 소유하며 결과 사실만 design.md로 환류한다 | 작업 트리와 카드 | v0.15.2 | 유효 |
...
| DD-52 | 공유 전이는 기억해 둔 통합 id에 대고 게시한다 | 동시성 · 점유 · 통합 | v0.13.0 | 유효 · 일부 정정 → DD-62 (v0.14.0), DD-70 (v0.16.0) |
...
| DD-70 | `coordinator`는 devflow의 단계가 아니라 선언된 역할이며, 그 의무는 역할 계약 하나가 소유하고 devflow의 동작·상태는 오케스트레이션 여부로 분기하지 않는다 | 동시성 · 점유 · 통합 | v0.16.0 | 유효 |
| DD-28 | 영속 지식은 새 문서층이 아니라 범위가 고정된 기존 기록의 소비 경로로 잇는다 | 지식층과 능력 문서 | v0.9.21 | 유효 · 일부 정정 → DD-76 (v0.18.1) |
...
| DD-42 | 능력 지식 기준선 — 검증 폐쇄가 낳는 도메인 설계도 | 지식층과 능력 문서 | v0.10.0 | 유효 · 일부 정정 → DD-76 (v0.18.1) |
| DD-43 | 능력 문서는 Layer 0에서 태어나는 설계 구역과 폐쇄에서 갱신되는 검증 구역을 물리적으로 나누며 항상 켠다 | 지식층과 능력 문서 | v0.11.0 | 유효 · 일부 정정 → DD-76 (v0.18.1) |
| DD-44 | 도메인 도달은 카드 필드가 아니라 깊이 1 번호 규칙과 resume의 도메인 진입 분기가 소유한다 | 지식층과 능력 문서 | v0.11.0 | 유효 · 일부 정정 → DD-76 (v0.18.1) |
...
| DD-67 | 기획 증거 규율은 네 권위의 사실을 질문 전에 닫고, 답만 필요한 조사는 격리하되 구조 이해와 구속 결정은 메인이 소유한다 | 지식층과 능력 문서 | v0.15.0 | 유효 |
| DD-73 | 능력 문서는 arch의 마지막 출력이며 첫 트리 개방 전에 존재해야 하고, 결정·외부 계약 근거는 확인 경계에서 보존한다 | 지식층과 능력 문서 | v0.17.0 | 유효 |
| DD-74 | 기획 기록층 — 기록 게이트가 결정·증거 기록을 정착 시점에 착지시킨다 | 지식층과 능력 문서 | v0.18.0 | 대체됨 → DD-77 (v0.18.2) |
| DD-75 | 기록의 승계 — 승계 하나가 현재를 바꾸고, 낡은 증거는 연쇄하며, 상충은 보존 뒤 병합된다 | 지식층과 능력 문서 | v0.18.0 | 대체됨 → DD-77 (v0.18.2) |
| DD-76 | 도메인 지식 캡슐 — 능력 문서를 넘치는 지식은 같은 번호 아래 필요 개봉 캡슐로 살고, 출처 표기가 원문·종합·추정·다툼을 가른다 | 지식층과 능력 문서 | v0.18.1 | 유효 · 일부 정정 → DD-77 (v0.18.2) |
| DD-77 | 문서는 현재만 말한다 — 같은 개념이 갱신되면 그 개념의 자리를 덮어쓰고, 버린 방향은 그 결론 옆에 현재 사실로 산다 | 지식층과 능력 문서 | v0.18.2 | 유효 |
| DD-78 | 정본 한 구간은 도구가 그 구간의 대상이 없음을 증명할 때만 읽지 않는다 — 기계가 가르고, HEAD와 워킹 트리를 둘 다 보고, 나머지 모든 답은 통독으로 무너진다 | 지식층과 능력 문서 | v0.18.5 | 유효 |
