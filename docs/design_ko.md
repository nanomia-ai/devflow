...
| `adopt` | 무관리 브라운필드의 모든 유지 자료를 Product·Architecture·해당 시 Design·code style·glossary·능력 설계 구역·완전한 영속 도메인 지식으로 명시적으로 투영하고, 의미 있는 사람 산문에서 작업 언어를 제안해 쓰기 전에 확인받음 | 무관리 문서 및/또는 코드 → 확인된 관리 지식 표면과 `product.md`의 작업 언어, 그 뒤 정지 | DD-10 · DD-20 · DD-26 · DD-97 · DD-107 |
