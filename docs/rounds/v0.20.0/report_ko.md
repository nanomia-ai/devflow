...
- Fable의 glossary release-gate 판정은 frozen brownfield 실물에서 실제 용어 6개가 canon 0개로
  사라지고 `Next`가 phantom term이 된 장면을 실행했다. Product가 영향받지 않는다는 앞선 주장도
...
수치가 아니다. `86a4d19`의 449/449와 `duration_ms 1279527.7144`도 이 glossary 수리에는
stale이며 새 wildcard 통과로 세지 않는다. 이 기록은 production readiness를 주장하지 않는다.
