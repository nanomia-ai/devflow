...
이번 수리의 실행 증거는 다음과 같다. wildcard root suite는 이 표에 없으며 아래 운영 경계에
계속 `unverified`로 남긴다.

| 항목 | 현재 결과 |
|---|---|
| Adopt fast lint | `L-fast: pass` |
| Adopt full build | L0–L18 pass · mutation 20/20 · scenario 31/31 × 200 repeats, mismatch 0 · format 2/2 |
| Adopt eval | L0–L18 pass · behavior 31/31 × 200 repeats, mismatch 0 |
| Product/Adopt collector·parser 집중 시험 | 8/8 pass — field map deep equality, template byte equality, 실제 Product collector `current`, canonical/legacy/mixed/blocking parser 경로 |
| repository invariants | 20/20 pass — 같은 기존 all-nine 검사에서 version과 hash를 함께 비교 |
| generated ownership/hash | 9 package owner mismatch 0 · runtime/validator identity variant 1 · generated hash 342/342 · Adopt/Product template SHA-256 `5bdea011440d1b74c9f375d8d4d9814f20fc6f9be22ea46be28def759034e649` |
| 문서·diff | decision index en/ko exit 0 · `git diff --check` exit 0 |

`docs/usecase-matrix_ko.md`의 H2/adopt 판정은 기획 증거 companion의 네 consumer와 조사 소유를
판정한다. 이번 수리는 그 진입·소비 집합을 바꾸지 않고 이미 정합인 Layer 0 출력 schema를 복원하므로
standing verdict는 그대로이며 matrix는 수정하지 않았다.
...
0으로 닫았다. 다음 검증 수단은 wildcard 반복이 아니라 별도 fresh worker의 full gate와 아래 Phase 4
실사용이다.
