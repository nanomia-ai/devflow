...
|---|---|---|---|
| `principles` | owns the shared policy index, commit discipline, and semantic-prose policy that follows the confirmed project Working language; it state-free-classifies only current-project devflow requests that enter Principles. A named stage reads that index from its own entry without Principles preflight | unnamed current-project devflow intent → resume; named stage → shared policy index then its named owner; role contract → contract directly | DD-03 · DD-29 · DD-57 · DD-92 · DD-93 · DD-97 · DD-107 |
| `product` | confirms the problem, identity, capabilities, boundary, success criteria, and project Working language with the owner, keeping that choice as one fact in `product.md` | explicit new-project or product-planning request → arch and design; initial existing-project reconstruction bypasses Product and belongs to Adopt | origin · DD-33 · DD-67 · DD-97 · DD-107 |
...
| `adopt` | explicitly accounts for all maintained brownfield sources, proposes the Working language from meaningful human prose for confirmation before writing, and projects the sources into Product, Architecture, applicable Design, code style, glossary, capability design zones, and complete durable domain knowledge | unmanaged documents and/or code → a confirmed managed planning and knowledge surface plus the `product.md` Working language, then stop | DD-10 · DD-20 · DD-26 · DD-97 · DD-107 |
