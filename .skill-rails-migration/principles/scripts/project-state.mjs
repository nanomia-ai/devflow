...
function exactSourceBasis(text, line) {
  if (text === null) return false;
  const escaped = `${line.sourceCard}@${line.sourceHash}`.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`^Source basis: .*\\"${escaped}:[1-9][0-9]*-[1-9][0-9]*\\"`, "m").test(text);
}
...
function knowledgeLandingState(snapshot) {
  ...
  for (const line of removedKnowledgeLandings(headText, snapshot.journalText ?? "")) {
    validateConsumption(line, workingChanged, null);
    effectiveLifecycles.add(line.raw);
  }

  const journalCommits = gitText(snapshot.root, ["log", "--format=%H", "--", "devflow/journal.md"], { allowFailure: true }).split("\n").filter(Boolean);
  for (const commit of journalCommits) {
    const parent = gitLine(snapshot.root, ["rev-parse", "--verify", `${commit}^`], { allowFailure: true });
    if (!parent) continue;
    const beforeText = gitFile(snapshot.root, parent, "devflow/journal.md") ?? "";
    const afterText = gitFile(snapshot.root, commit, "devflow/journal.md") ?? "";
    const removed = removedKnowledgeLandings(beforeText, afterText);
    if (removed.length === 0) continue;
    const changed = new Set(gitNulList(snapshot.root, ["diff", "--name-only", "-z", parent, commit, "--"]));
...
export async function calculateState(options) {
  ...
  return {
    ...
    metadata: {
      ...
      tree: snapshot.treePresent ? "present" : "absent", narrow: projected.narrow,
    },
