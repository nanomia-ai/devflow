...
이 프로젝트는 devflow로 관리된다. 사용자가 요청 의도를 밝힌 뒤 모델 호출이 가능한
`devflow:principles` 스킬을 실행하여 요청을 분류하고 그 경로를 그대로 따른다. devflow 역할
...
모델 호출이 가능한 principles 스킬이 없으면 코드를 고치지 않는다. 사용자에게 devflow 플러그인을
