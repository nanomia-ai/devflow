...
Purpose: provide one state-free request classifier and the shared policy/grammar kernel for every devflow entry, including the project-wide authoring-language contract.
Failure scene: duplicated hook-time state collection or scattered prose rules make sessions drift, re-enter stages, mix semantic prose languages, translate fixed machine tokens, or accept malformed durable records.
...
Ownership: spec owns routing and exact machine forms; body owns judgment reasons; templates own output grammar; selected references own topic policy, including the Product-owned Working language behavior, input-owner K discovery, and the shared owner/K approval and physical-writer boundaries; sibling state tools own deterministic project facts.
