...
export const STAGES = [
  ...
  { id: "proposal", reads: ["proposal.completeness"], acceptsUnknown: [], done: s => s.proposal.completeness === "ready", needs: ["proposal.completeness"], reentry: "rejudge", branches: { "needs-choice": [["RUN", { action: "project existing product/K and arch/K headers separately under each exact input owner; open only K whose first-line use-when matches a pending Design judgment" }], ["RUN", { action: "project existing design/K headers under the exact design owner" }], ["REPORT", { template: "proposal" }], "ASK"] }, body: "stage: proposal" },
  { id: "confirmation", reads: ["current.design", "request.kind"], acceptsUnknown: [], done: s => s.current.design === "present" && (s.request.kind === "none" || s.request.kind === "tiny-no-delta"), needs: ["approval.action"], reentry: "rejudge", branches: {
    ask: [["RUN", { action: "project existing product/K and arch/K headers separately under each exact input owner; open only K whose first-line use-when matches a pending Design judgment" }], ["RUN", { action: "project existing design/K headers under the exact design owner" }], ["REPORT", { template: "proposal" }], "ASK"],
...
export const ARTIFACTS = {
  ...
  productKnowledge: { path: ".devflow/project/product", writer: "external.product", readers: ["stage.proposal", "stage.confirmation"] },
  ...
  archKnowledge: { path: ".devflow/project/arch", writer: "external.arch", readers: ["stage.proposal", "stage.confirmation"] },
