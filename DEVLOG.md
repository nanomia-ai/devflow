...
- **팀 모드 (v0.8 유력 후보)** — 발동 조건: 실제 팀원이 이 스킬을 같은 저장소에 쓰게 될 때.
  방향은 확정해 둠(2026-08-07 검토): ① `.wip.`는 전역 1개 → 1인 1개로 재해석(소유는 journal
  기록에 이름) ② HANDOFF는 `HANDOFF-<이름>.md`로 사용자별 분리 — 단 journal·트리·핵심 문서는
  공유 유지(사람별 분리는 "같은 주제 두 문서"를 자초) ③ 번호 발급은 통합 브랜치의 split에서만
  — 대기 카드 선커밋으로 선점, `02.2b`가 완충 ④ 능력 폐쇄(verify)와 핵심 문서 수정은 통합
  브랜치의 행위. 토대(카드당 파일·번호 불변·git 정본)는 이미 병합 친화적이라 비가역 장애물
  없음. 선행 사례: Beads(항목당 파일·해시 ID·worktree 병합), Superpowers는 미해결(브랜치로 환원).
